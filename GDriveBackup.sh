...
create_backup_zip() {
    ...
    zip -r "$backup_file_name" ./* #TODO: ensure availability
    echo "$backup_file_name"
}

upload_to_drive() {
    local file="$1"
    local backup_folder="BACKUP" #TODO: accepting custom flags
...
if [ ! -f "${HOME}/.googledrive.conf" ]; then #Checking for config file
    ...
    if [ "$conf" == "y" ]; then
        ...
        gupload --config "$path" "$backup_file" #TODO: check for existence first
    else
...
rm "$backup_file" #TODO: Ensure existence
